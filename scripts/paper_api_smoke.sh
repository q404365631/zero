...
NETWORK_PROFILE="$(mktemp)"
curl -fsS "${API}/network/profile" >"${NETWORK_PROFILE}"
"${PYTHON_BIN}" - "${NETWORK_PROFILE}" "${IDENTITY_CLAIM}" "${IDENTITY_HEARTBEAT}" <<'PY'
import json
import sys
