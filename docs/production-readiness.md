...
| CLI readiness | 95 | Mature Rust terminal, doctor, TUI, friction gates, tests, release binary path, recovery-aware status output, live-preflight diagnostics, live risk-reducer wiring, `/live-cockpit`, `/live-certify`, `/immune`, and friction-gated `/resume-entries` operator readouts exist. Remaining full-screen cockpit drills need real canary evidence. |
...
| Diagnostics | 91 | Doctor, JSON output, exit codes, rate-budget checks, live-preflight diagnostics, live-cockpit next-action rendering, and live-control refusals are strong. Railway remote-log automation is still missing. |
