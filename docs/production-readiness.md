...
| Engine integration | 94 | HTTP, WebSocket, mock engine, contract tests, Rust client decoding for production-parity OODA reports, `/runtime-parity` operator rendering, and live risk-reducer endpoints exist. Real accepted canary evidence remains external. |
...
| Non-interactive automation | 90 | `zero run` covers cockpit, runtime parity, breaker, certification, account truth, and risk-reducer workflows while intentionally gating risk-increasing commands. Needs external production examples. |
